...
namespace CairoEngine.InventoryManagement
{
	public class InventoryModule : MonoBehaviour
	{
		...
		{
			SaveSystemModule.SetProperty("Inventory_" + inventory.template.ID, inventory.items);
		}
		...
		{
			inventory.items = (List<DriverTemplate_InventoryItem>)SaveSystemModule.GetProperty("Inventory_" + inventory.template.ID);
		}
