...
namespace CairoEngine
{
    ...
    public class StateMachineModule
    {
        ...
        public static void SetState(GameObject stateGameObject, string state, string behaviourName = "")
        {
            if (fsmObjects.ContainsKey(stateGameObject))
            {
                if (behaviourName == "")
                {
                    ...
                    {
                        List<object> blockedLists = GetFields(stateGameObject, "blockedList");
                        bool blocked = false;

                        //Loop through all the Blocked Lists in each State with with the given name
                        foreach (object blockedList in blockedLists)
                        {
                            List<string> blockedListString = ((string)blockedList).ParseToList();

                            foreach (string blockedState in blockedListString)
                            {
                                if (((string)blockedList) == state)
                                {
                                    blocked = true;
                                }
                            }
        ...
        public static List<object> GetFields(GameObject stateGameObject, string fieldName)
        {
            ...
            foreach(StateContainer stateContainers in fsmObjects[stateGameObject])
            {
                ...
                foreach(string stateName in stateContainers.states.Keys)
                {
                    if(stateName == stateContainers.curState)
                    {
                        ...
                        if (field != null)
                            values.Add(field.GetValue(stateContainers.states[stateName]));
                    }
                }
            }
