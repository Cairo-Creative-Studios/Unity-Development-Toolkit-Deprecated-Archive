...
{
    /// <summary>
    /// Controls the Game Mode that is used to Play the Game. Using the Game Mode Manager isn't necessary for building Gameplay, but it vastly simplifies development of Game Mode centric Game development.
    /// </summary>
    public class GameModeManager
    {
        ...
        {
            //Initialzie the Game Mode Manager with the Template Game Mode Info
            if (!firstGameModeSet)
                if (gameMode == null)
        ...
        {
            GameModeManager.gameMode = FindGameModeInfo(gameModeName);
            if (gameMode != null)
            ...
            EntityManager.SpawnPawn(gameMode.defaultPawnInfo, LevelManager.spawnPoints[0].transform.position);
        }
        ...
        {
            GameModeManager.teamScores[team] += points;
        }
